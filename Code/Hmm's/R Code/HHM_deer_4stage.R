...
mu0 <- c(80, 12, 0.1,140) 
sigma0 <- c(35, 11, 0.1,35) 
zeromass0 <- c(0.1, 0, 0.9,0.1) 
...
########################################################
readRDS(hmm_model, file = paste0("temp", ".rds"))
# Zustände zuweisen
states <- viterbi(hmm_model)
hmm_data$state <- states
data_short$state <- states
data$state<- states
...
state_counts <- table(data_short$state)
...
stillstand_count <- sum(data_short$state == 3)
...
print(state_proportions)
#########################################################
saveRDS(hmm_model, file = paste0("temp", ".rds"))
########################################################
ggsave(
